...
from pysrc.prediction.network.sparse_network_test import *
...
class MatrixGeneralValueFunctionNetwork(object):
    ...
    def __init__(self, function_approx, num_predictions, step_sizes, discounts, cumulants, traces_lambda,
        ...
        if remove_base:
            ...
            self.max_obs = np.ones(self.function_approximation.dimensions + self.protected_range)
            self.min_obs = np.ones(self.function_approximation.dimensions + self.protected_range)
        ...
        self.weights = csc_matrix((num_predictions, self.function_approximation.numPrototypes + 1))
        self.step_sizes = np.ones((num_predictions, self.function_approximation.numPrototypes + 1)) * step_sizes[:,
                                                                                                      None]
        self.eligibility_traces = csc_matrix(self.weights.shape)
        ...
        self.h = csc_matrix((self.beta.shape))
        self.n = csc_matrix((self.beta.shape))

        # Error and internal measurement
        self.estimated_return = np.zeros((num_predictions))
        self.avg_error = np.zeros((num_predictions))
        self.synced_prediction = np.zeros((num_predictions))
        ...
        self.reward_history = csc_matrix((num_predictions, horizon))
        self.gamma_history = csc_matrix((num_predictions, horizon))
        self.prediction_history = csc_matrix((num_predictions, horizon))
        ...
        self.rupee_h_trace = csc_matrix(self.eligibility_traces.shape)
        self.eligibility_avg = csc_matrix(self.eligibility_traces.shape)
        self.last_phi = None
    ...
    def generate_prediction(self, locations, obs):
        ...
        for i in locations:
            ...
            self.beta[i] = np.ones(self.beta[i].shape) * np.log(1. / self.function_approximation.numActiveFeatures)
    ...
    def terminal_step(self, observations, remove_base, **vals):
        """Update the Network"""
        ...
        observations += self.min_obs
        observations = np.divide(observations, self.max_obs - self.min_obs,
                                 where=(self.max_obs - self.min_obs) != 0)
        ...
        if type(self.last_phi) is np.ndarray:
            ...
            self.eligibility_traces = accumulate(self.eligibility_traces, self.discounts, self.traces_lambda,
                                                 phi_next)
            ...
            td_error = calculate_temporal_difference_error(self.weights, current_cumulants,
                                                           np.zeros(self.discounts.shape),
                                                           phi_next, self.last_phi)
            ...
            if self.use_step_sizes:
                ...
                self.step_sizes = normalize_step_size(
                    calculate_step_size(self.beta), self.beta, self.eligibility_traces, self.discounts, self.last_phi,
                    phi_next
                )
    ...
    def step(self, observations, remove_base=False, **vals):
        ...
        if type(self.last_phi) is np.ndarray:
            ...
            current_cumulants = np.array([cumulant.cumulant(observations) for cumulant in self.cumulants])[:, None]
            # get a vector of TD errors corresponding to the performance.
            td_error = calculate_temporal_difference_error(self.weights, current_cumulants, self.discounts, phi_next,
                                                           self.last_phi)
            # update the weights based on the caluculated TD error
            predictions = self.predict(phi_next)
            # update the running trace of maximum meta_weight updates
            if self.use_step_sizes:
                ...
                self.beta = update_meta_weights(self.beta, self.last_phi, self.meta_step_size, td_error, self.h, self.n)
                self.step_sizes = normalize_step_size(
                    calculate_step_size(self.beta), self.beta, self.eligibility_traces, self.discounts, self.last_phi,
                    phi_next
                )
    ...
    def initalize_episode(self):
        self.eligibility_traces = csc_matrix((self.eligibility_traces.shape))
        self.h = csc_matrix(self.h.shape)
        self.last_phi = None
        self.prediction_history = csc_matrix((self.prediction_history.shape))
        self.gamma_history = csc_matrix((self.gamma_history.shape))
        self.reward_history = csc_matrix((self.reward_history.shape))
        self.delta_var = None
        self.delta_avg = None
        self.rupee_h_trace = csc_matrix((self.rupee_h_trace.shape))
        self.rupee_beta = 0.01
        self.tau = np.ones(self.tau.shape) * 0.001
        self.eligibility_avg = csc_matrix((self.eligibility_avg.shape))
...
class HierarchicalMatrixNetwork(MatrixGeneralValueFunctionNetwork):
    ...
    def generate_state(observations, layer, predictions=None, recurrance=False, skip=False, ude=False,
        ...
        Args:
            ...
            layer (MatrixGeneralValueFunctionNetwork): the present layer
            """
    ...
    def terminal_step(self, observations, recurrance=False, skip=False, ude=False, remove_base=False, **vals):
        """Update the Network"""
        # get the next feature vector
        predictions = None
        for layer in self.layers:
            state = self.generate_state(observations, layer, predictions, recurrance, skip, ude, remove_base)
            predictions = layer.terminal_step(state, remove_base)
        return predictions
    ...
    def seed_predictions(self):
        for layer in self.layers:
            layer.generate_prediction(range(layer.protected_range, layer.weights.shape[0]), layer.min_obs)
