...
def accumulate(traces, gamma, lmbda, phi):
    ...
    Returns:
        traces: the eligibility traces to asign credit for current observations to previous states.
    """
    a = lil_matrix(traces.shape)
    a.setdiag(gamma*lmbda)
    b = lil_matrix(traces.shape)
    b.setdiag(phi)
    print "the shape of things", (traces * a).shape
    # return traces * a.transpose() + b
...
def calculate_temporal_difference_error(weights, cumulant, gamma_next, phi_next, phi):
    ...
    """
    return cumulant + gamma_next * weights[phi_next].sum() - weights[phi].sum()
...
def update_normalizer_accumulation(normalizer, beta, traces, phi, h, td_error, tau=1. / 1000.):
    ...
    return np.maximum(update, + (1. / tau) * np.exp(beta) * traces * phi.T * update - normalizer)


def normalize_meta_weights(beta, traces, gamma, phi, phi_next):
    """normalizes 
    Args:
        beta: meta-weights which define step-sizes.
        traces: the eligibility traces to asign credit for current observations to previous states.
    ...
    return beta - np.log(m)


def update_meta_weights(beta, phi, meta_step_size, td_error, h):
    ...
    Returns:
        beta: meta-weights which define step-sizes.
    """
    return beta + (meta_step_size * td_error * h * phi.T)
...
def update_meta_weight_update_trace(h, traces, phi, td_error, step_size):
    ...
    """
    h = (np.ones(traces.shape) - step_size * phi.T * traces) * h
    h[h < 0] = 0
