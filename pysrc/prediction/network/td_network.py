...
class MatrixGeneralValueFunctionNetwork(object):
    ...
    def __init__(self, function_approx, num_predictions, step_sizes, discounts, cumulants, traces_lambda, protected_range=0, recurrance=False, skip=False, remove_base=False, use_step_size=False, **vals):
        ...
        self.step_sizes = np.ones((num_predictions, self.function_approximation.numPrototypes+1)) * step_sizes[:,None]
    ...
    def terminal_step(self, observations, remove_base, **vals):
        ...
        if type(self.last_phi) is np.ndarray:
            ...
            td_error = calculate_temporal_difference_error(self.weights, current_cumulants, np.zeros(self.discounts.shape),
    ...
    def step(self, observations, remove_base=False, **vals):
        ...
        if type(self.last_phi) is np.ndarray:
            ...
            if self.use_step_sizes:
                ...
                self.step_sizes = normalize_step_size(
                    calculate_step_size(self.beta), self.beta, self.eligibility_traces, self.discounts, self.last_phi, phi_next
                )
                # print np.average(self.step_sizes, axis=1)
    ...
    def initalize_episode(self):
        ...
        self.last_phi = None
        self.prediction_history = np.zeros(shape=(self.prediction_history.shape))
        self.gamma_history = np.zeros(shape=(self.gamma_history.shape))
        self.reward_history = np.zeros(shape=(self.reward_history.shape))
        self.delta_var = None
        self.delta_avg = None
